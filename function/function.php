...
function site_info() {
	...
	$select = mysql_fetch_object(mysql_query("SELECT * FROM `".$prefix."setting` WHERE id='1'"));
	return $select;
}
// site details
function home_info() {
	...
	$select = mysql_fetch_object(mysql_query("SELECT * FROM `".$prefix."home_setting` WHERE id='1'"));
	return $select;
}
...
function row($table,$id) {
	...
	$select = mysql_fetch_object(mysql_query("SELECT * FROM `".$prefix.$table."` WHERE id='".$id."'"));
	return $select;
}
function rowCondition($table,$field,$value) {
	...
	$select = mysql_fetch_object(mysql_query("SELECT * FROM `".$prefix.$table."` WHERE `".$field."`='".$value."'"));
	return $select;
}
...
function del($id,$table) {
	...
	$sql = mysql_query("DELETE FROM `".$prefix.$table."` WHERE id='".$id."'");
	return $sql;
...
function del_condition($value,$table,$field) {
	...
	$sql = mysql_query("DELETE FROM `".$prefix.$table."` WHERE `".$field."`='".$value."'");
	return $sql;
...
function count_rows($table, $field, $value) {
	...
	$select = mysql_query("SELECT * FROM `".$prefix.$table."` WHERE ".$field."='".$value."'");
...
function anything($table,$field,$id) {
	...
	$select = mysql_query("SELECT * FROM `".$prefix.$table."` WHERE id='".$id."'");
...
function setData($table,$field,$data,$wfield,$wdata,$query=''){
	...
	if($query == '' && count_rows($table, $wfield, $wdata)> 0){
		if(mysql_query("UPDATE `".$prefix.$table."` SET `".$field."`='".$data."' WHERE `".$wfield."`='".$wdata."'"))
			return true;
		else return false;
	}
	else{
		if(mysql_query("INSERT INTO `".$prefix.$table."` SET ".$query))
			return true;
		else return false;
	}
}
...
function getDefImg($id){
	...
	$select = mysql_query("SELECT `image` FROM `".$prefix."product_img` WHERE `default_img`='1' AND `pro_id`='".$id."'");
	$row = mysql_fetch_object($select);
	if(mysql_num_rows($select) == 1)
		return $row->image;
	else return false;
...
function getStock($id){
	...
	$select = mysql_query("SELECT `stock` FROM `".$prefix."product` WHERE `id`='".$id."'");
	$row = mysql_fetch_object($select);
...
function getAmount($table, $field_name){
	...
	$select1 = mysql_query("SELECT `".$field_name."` FROM `".$prefix.$table."` WHERE `paid`='0'");
	$i=0;
...
function getProductsales(){
	...
	$select = mysql_query("SELECT `qty` FROM `".$prefix."product_order` WHERE `paid`='0'");
	$i=0;
